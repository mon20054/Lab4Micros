...
; Contador con TIMR0 en puerto E
; Hardware: LEDs en el puerto A y E
; Display de 7 segmentos en el puerto C y D
;
; Creado: 12 febrero 2022
; Última modificación: 12 febrero 2022
...
isr:
    btfss PORTB, 0 ; Ver si boton 1 fue presionado
    call antirebotes1
    btfss PORTB, 1 ; Ver si boton 2 fue presionado
    call antirebotes2
    banksel INTCON
    btfsc ((INTCON) and 07Fh), 2 ; Ver si bandera de TMR0 se encendió
    call t0
    bcf ((INTCON) and 07Fh), 0 ; Limpiamos bandera de interrupcion del puerto B
...
loop:
    ...
    btfsc cont_2, 4 ; Verifica si contador 1 es mayor a 15
    call complete1
    btfsc cont_3, 4 ; Verifica si contador 3 es mayor a 15
    call complete2
    movf decenas,0 ; Se mueve el valor de decenas a W
    subwf ten, 0 ; Se verifica si las decenas alcanzaron 10
    ...
    movf decenas,0 ; Se mueve el valor de decenas a W
    movwf offset ; Se almacena el valor de decenas en offset
    movf offset, 0 ; Se almacena el offset en W
    call tabla
    movwf binario ; Se almacena el valor obetnido de la tabla en el registro binario
    movf binario, 0 ; Se mueve el valor a W
...
config_tmr0:
    ...
    bsf OPTION_REG, 2 ; ((OPTION_REG) and 07Fh), 2
    bsf OPTION_REG, 1 ; ((OPTION_REG) and 07Fh), 1
    bsf OPTION_REG, 0 ; ((OPTION_REG) and 07Fh), 0 Prescaler de 1 : 256
    banksel TMR0 ; Cambiamos a banco 0 de TIMER0
    movlw 252 ; Cargamos el valor 191 a W
    movwf TMR0 ; Cargamos el valor de W a TIMER0 para 66.66mS de delay
...
config_IO:
    ...
    banksel TRISB
    bsf TRISB, 0 ; ((PORTB) and 07Fh), 0 como entrada
    bsf TRISB, 1 ; ((PORTB) and 07Fh), 1 como entrada
    banksel WPUB ; Banco para pullups puerto B
    bsf WPUB, 0 ; ((PORTB) and 07Fh), 0 con Pullup interno
    bsf WPUB, 1 ; ((PORTB) and 07Fh), 1 con Pullup interno
    banksel TRISA
